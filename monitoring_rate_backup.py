...
# System metrics endpoint@monitoring_bp.route("/system", methods=["GET"])def system_metrics_endpoint():    """Sistem metrikleri endpoint"i"""    try:        metrics = get_system_metrics()        if "error" in metrics:            return jsonify(metrics), 500        return jsonify({            "status": "success",            "data": metrics,            "timestamp": datetime.utcnow().isoformat()        })    except Exception as e:        return jsonify({            "status": "error",            "error": str(e),            "timestamp": datetime.utcnow().isoformat()        }), 500
...
@monitoring_bp.route("/app", methods=["GET"])
def app_metrics_endpoint():
    """Uygulama metrikleri endpoint"i"""
    try:
...
@monitoring_bp.route("/business", methods=["GET"])
def business_metrics_endpoint():
    """İş metrikleri endpoint"i"""
    try:
...
# Rate limits metrics endpoint
@monitoring_bp.route("/rate-limits", methods=["GET"])
def rate_limits_metrics_endpoint():
    """Rate limits metrikleri endpoint"i"""
    try:
        metrics = get_rate_limiting_metrics()
        if "error" in metrics:
            return jsonify(metrics), 500
        return jsonify({"status": "success", "data": metrics})
    except Exception as e:
